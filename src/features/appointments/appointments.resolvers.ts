...
export const appointmentsResolvers = {
  Query: {
    appointment: async (
      ...
      return appointmentsService.getAppointment(
        id,
        context.user!.id,
        context.user!.role,
      );
  ...
  Mutation: {
    ...
    removeAppointment: async (
      _: any,
      { input }: RemoveAppointmentArgs,
      context: GraphQLContext,
    ) => {
      return appointmentsService.removeAppointment(
        input,
        context.user!.id,
        context.user!.role,
      );
  ...
  Appointment: {
    ...
    status: (appointment: Appointment) =>
      appointmentsService.getAppointmentStatus(appointment),
  },
};
