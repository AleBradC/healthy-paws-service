...
describe("petsResolvers", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(petsService.createPet)
      .mockReset()
      .mockResolvedValue({ id: "new-pet", name: "Buddy" } as unknown as Pet);
    vi.mocked(petsService.updatePet)
      .mockReset()
      .mockResolvedValue({
        id: "pet-1",
        name: "Updated Buddy",
  ...
  describe("Query.pet", () => {
    it("should call petsService.getPet and load the pet", async () => {
      ...
      vi.mocked(mockContext.petLoaders.petById.load).mockResolvedValue(
        mockPet as any,
      );

      const result = await petsResolvers.Query.pet(
        null,
        { id: petId },
      ...
      expect(
        vi.mocked(mockContext.petLoaders.petById.load),
      ).toHaveBeenCalledWith(petId);
  ...
  describe("Mutation.createPet", () => {
    it("should call petsService.createPet and return a new pet", async () => {
      ...
      const result = await petsResolvers.Mutation.createPet(
        null,
        { input },
        mockContext as GraphQLContext,
      );
  ...
  describe("Mutation.updatePet", () => {
    it("should call petsService.updatePet and update the pet", async () => {
      ...
      const result = await petsResolvers.Mutation.updatePet(
        null,
        { input },
        mockContext as GraphQLContext,
      );
